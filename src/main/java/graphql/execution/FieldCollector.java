...
public class FieldCollector {
  ...
  public void collectFields(ExecutionContext executionContext, GraphQLObjectType type,
    ...
    for (Selection selection : selectionSet.getSelections()) {
      if (selection instanceof Field) {
        ...
        collectInlineFragment(executionContext, type, visitedFragments, fields,
            (InlineFragment) selection);
      } else if (selection instanceof FragmentSpread) {
        collectFragmentSpread(executionContext, type, visitedFragments, fields,
            (FragmentSpread) selection);
      }
    }
  }

  private void collectFragmentSpread(ExecutionContext executionContext, GraphQLObjectType type,
      List<String> visitedFragments, Map<String, List<Field>> fields,
    ...
    collectFields(executionContext, type, fragmentDefinition.getSelectionSet(), visitedFragments,
        fields);
  }

  private void collectInlineFragment(ExecutionContext executionContext, GraphQLObjectType type,
      List<String> visitedFragments, Map<String, List<Field>> fields,
    ...
    collectFields(executionContext, type, inlineFragment.getSelectionSet(), visitedFragments,
        fields);
  }
  ...
  private boolean doesFragmentConditionMatch(ExecutionContext executionContext,
    ...
    conditionType =
        getTypeFromAST(executionContext.getGraphQLSchema(), inlineFragment.getTypeCondition());
    return checkTypeCondition(executionContext, type, conditionType);
  }

  private boolean doesFragmentConditionMatch(ExecutionContext executionContext,
      FragmentDefinition fragmentDefinition, GraphQLObjectType type) {
    GraphQLType conditionType;
    conditionType =
        getTypeFromAST(executionContext.getGraphQLSchema(), fragmentDefinition.getTypeCondition());
    return checkTypeCondition(executionContext, type, conditionType);
  }

  private boolean checkTypeCondition(ExecutionContext executionContext, GraphQLObjectType type,
      GraphQLType conditionType) {
    ...
    if (conditionType instanceof GraphQLInterfaceType) {
      List<GraphQLObjectType> implementations = SchemaUtil.findImplementations(
          executionContext.getGraphQLSchema(), (GraphQLInterfaceType) conditionType);
