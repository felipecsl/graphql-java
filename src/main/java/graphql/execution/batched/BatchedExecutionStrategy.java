...
public class BatchedExecutionStrategy extends ExecutionStrategy {
  ...
  private List<GraphQLExecutionNode> resolveField(ExecutionContext executionContext,
    ...
    List<GraphQLExecutionNodeValue> values =
        fetchData(executionContext, parentType, nodeData, fields, fieldDef);

    return completeValues(executionContext, parentType, values, fieldName, fields,
        fieldDef.getType());
  }
   ...
   */
  private List<GraphQLExecutionNode> completeValues(ExecutionContext executionContext,
      GraphQLObjectType parentType, List<GraphQLExecutionNodeValue> values, String fieldName,
      List<Field> fields, GraphQLOutputType outputType) {
    ...
    if (isPrimitive(fieldType)) {
      ...
      return handleList(executionContext, values, fieldName, fields, parentType,
  ...
  private List<GraphQLExecutionNode> handleList(ExecutionContext executionContext,
    ...
    List<GraphQLExecutionNodeValue> flattenedNodeValues =
        new ArrayList<GraphQLExecutionNodeValue>();

    for (GraphQLExecutionNodeValue value : values) {
      if (value.getValue() == null) {
        value.getResultContainer().putResult(fieldName, null);
      } else {
        GraphQLExecutionResultList flattenedDatum =
            value.getResultContainer().createAndPutEmptyChildList(fieldName);
    ...
    return completeValues(executionContext, parentType, flattenedNodeValues, fieldName, fields,
        subType);
  ...
  private List<GraphQLExecutionNode> createChildNodes(ExecutionContext executionContext,
      List<Field> fields, ChildDataCollector collector) {
    ...
    for (ChildDataCollector.Entry entry : collector.getEntries()) {
      Map<String, List<Field>> childFields =
          getChildFields(executionContext, entry.getObjectType(), fields);
      childNodes.add(new GraphQLExecutionNode(entry.getObjectType(), childFields, entry.getData()));
    }
    return childNodes;
  }

  private ChildDataCollector createAndPopulateChildData(List<GraphQLExecutionNodeValue> values,
      String fieldName, GraphQLType fieldType) {
    ...
    for (GraphQLExecutionNodeValue value : values) {
      if (value.getValue() == null) {
        ...
        GraphQLExecutionNodeDatum childDatum =
            value.getResultContainer().createAndPutChildDatum(fieldName, value.getValue());
  ...
  private GraphQLType handleNonNullType(GraphQLType fieldType,
      List<GraphQLExecutionNodeValue> values,
      ...
      for (GraphQLExecutionNodeValue value : values) {
        if (value.getValue() == null) {
          throw new GraphQLException(
              "Found null value for non-null type with parent: '" + parentType.getName() +
                  "' for fields: " + fields);
        }
      }
  ...
  @SuppressWarnings("unchecked")
  private List<GraphQLExecutionNodeValue> fetchData(ExecutionContext executionContext,
      GraphQLObjectType parentType, List<GraphQLExecutionNodeDatum> nodeData, List<Field> fields,
      GraphQLFieldDefinition fieldDef) {
    ...
    DataFetchingEnvironment environment =
        new DataFetchingEnvironment(sources, argumentValues, executionContext.getRoot(), fields,
