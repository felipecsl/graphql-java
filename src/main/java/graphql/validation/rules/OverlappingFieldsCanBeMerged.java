...
public class OverlappingFieldsCanBeMerged extends AbstractRule {
  ...
  private Conflict findConflict(String responseName, FieldAndType fieldAndType1,
      FieldAndType fieldAndType2) {
    ...
    if (!fieldName1.equals(fieldName2)) {
      String reason =
          String.format("%s: %s and %s are different fields", responseName, fieldName1, fieldName2);
  ...
  private void collectFields(Map<String, List<FieldAndType>> fieldMap, SelectionSet selectionSet,
    ...
    for (Selection selection : selectionSet.getSelections()) {
      if (selection instanceof Field) {
        ...
        collectFieldsForInlineFragment(fieldMap, visitedFragmentSpreads,
            (InlineFragment) selection);

      } else if (selection instanceof FragmentSpread) {
        collectFieldsForFragmentSpread(fieldMap, visitedFragmentSpreads,
  ...
  private void collectFieldsForFragmentSpread(Map<String, List<FieldAndType>> fieldMap,
    ...
    GraphQLOutputType graphQLType = (GraphQLOutputType) TypeFromAST
        .getTypeFromAST(getValidationContext().getSchema(), fragment.getTypeCondition());
