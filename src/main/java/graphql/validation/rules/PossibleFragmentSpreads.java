...
public class PossibleFragmentSpreads extends AbstractRule {
  ...
  public void checkFragmentSpread(FragmentSpread fragmentSpread) {
    ...
    GraphQLType typeCondition =
        TypeFromAST.getTypeFromAST(getValidationContext().getSchema(), fragment.getTypeCondition());
    GraphQLCompositeType parentType = getValidationContext().getParentType();
    ...
    if (!doTypesOverlap(typeCondition, parentType)) {
      String message = String.format(
          ...
          fragmentSpread.getName(), parentType, typeCondition);
      addError(new ValidationError(ValidationErrorType.InvalidFragmentType,
          fragmentSpread.getSourceLocation(), message));
    }
  }
