...
public class TraversalContext implements QueryLanguageVisitor {
  ...
  private void enterImpl(ObjectField objectField) {
    ...
    if (objectType instanceof GraphQLInputObjectType) {
      GraphQLInputObjectField inputField =
          ((GraphQLInputObjectType) objectType).getField(objectField.getName());
      if (inputField != null) inputType = inputField.getType();
    }
    addInputType(inputType);
  }
  ...
  private GraphQLFieldDefinition getFieldDef(GraphQLSchema schema, GraphQLType parentType,
    ...
    if (field.getName().equals(TypeNameMetaFieldDef.getName()) &&
        (parentType instanceof GraphQLObjectType || parentType instanceof GraphQLInterfaceType ||
