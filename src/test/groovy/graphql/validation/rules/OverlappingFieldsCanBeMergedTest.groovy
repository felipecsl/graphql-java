...
class OverlappingFieldsCanBeMergedTest extends Specification {
  ...
  def traverse(String query, GraphQLSchema schema) {
    if (schema == null) {
      def objectType = GraphQLObjectType.newBuilder()
          .name("Test")
          .field(GraphQLFieldDefinition.newBuilder()
          .name("name")
          .type(GraphQLString))
          .field(GraphQLFieldDefinition.newBuilder().name("nickname").type(GraphQLString))
          .build();
  ...
  GraphQLSchema unionSchema() {
    def StringBox = newBuilder().name("StringBox")
        .field(GraphQLFieldDefinition.newBuilder().name("scalar").type(GraphQLString))
        .build()
    def IntBox = newBuilder().name("IntBox")
        .field(GraphQLFieldDefinition.newBuilder().name("scalar").type(GraphQLInt))
        .build()

    def NonNullStringBox1 = newBuilder().name("NonNullStringBox1")
        .field(GraphQLFieldDefinition.newBuilder().name("scalar").type(new GraphQLNonNull(GraphQLString)))
        .build()

    def NonNullStringBox2 = newBuilder().name("NonNullStringBox2")
        .field(GraphQLFieldDefinition.newBuilder()
        .name("scalar")
